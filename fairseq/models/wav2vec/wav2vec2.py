...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def compute_preds(self, x, y, negatives):
        ...
        # print('x:', x.size(), 'y', y.size()) #x: torch.Size([5, 368, 256]) y torch.Size([5, 368, 256])
        ...
        # print('negatives.size():', negatives.size()) # negatives.size(): torch.Size([100, 5, 353, 256])
        # print('neg_is_pos.size():', neg_is_pos.size()) # neg_is_pos.size(): torch.Size([100, 5, 353])
        # print('negatives:', negatives)
        # print('neg_is_pos:', neg_is_pos)
        
        # print('self.logit_temp:', self.logit_temp) # self.logit_temp: 0.1

        # print('targets:', targets.size(), 'logits', logits.size()) # targets: torch.Size([101, 5, 368, 256]) logits torch.Size([101, 5, 368])
    ...
    def forward(
        ...
        # print('features', features.size()) # features torch.Size([5, 512, 781]) # B,F,T

        features = features.transpose(1, 2)
        features = self.layer_norm(features)
        unmasked_features = features.clone()

        # raise ValueError('padding_mask:', padding_mask) # ('padding_mask:', None)
        ...
        # raise ValueError('time_steps_to_drop:', time_steps_to_drop, 'self.crop_seq_to_multiple', self.crop_seq_to_multiple) 
        # ('time_steps_to_drop:', 0, 'self.crop_seq_to_multiple', 1)
        ...
        # raise ValueError('self.input_quantizer:', self.input_quantizer, 'mask:', mask) # ('self.input_quantizer:', None, 'mask:', True)
        ...
        # raise ValueError('self.encoder:', self.encoder, 'self.post_extract_proj', self.post_extract_proj) # 'self.encoder:', TransformerEncoder
        ...
        if self.quantizer:
            if self.negatives_from_everywhere:
                ...
                prob_ppl = q["prob_perplexity"]
                curr_temp = q["temp"]
                loss_ent_max = q["loss_ent_max"]

                # raise ValueError('q', q["x"].size()) # 'y', torch.Size([5, 357, 256])
                # 'x': tensor([[[0.8242, 0.6968, 0.6772,  ..., 0.0341, 0.2688, 0.3333],
                #         [0.4185, 0.1895, 0.8716,  ..., 0.8594, 0.1016, 0.4343],
                #         [0.7930, 0.2676, 0.4060,  ..., 0.1949, 0.8403, 0.5059],
                #         ...,
                y = self.project_q(y)
            ...
            # raise ValueError('self.codebook_negatives:', self.codebook_negatives, 'self.negatives_from_everywhere:', self.negatives_from_everywhere, 'self.quantizer:', self.quantizer)
            # ValueError: ('self.codebook_negatives:', 0, 'self.negatives_from_everywhere:', False, 'self.quantizer:', GumbelVectorQuantizer(
        ...
        # raise ValueError('x', x.size(), 'y', y.size()) # ValueError: ('x', torch.Size([101, 5, 363]), 'y', torch.Size([5, 363, 256]))
