...
class Wav2vecCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss_ent_max = net_output["loss_ent_max"]
        ...
        # print('tar', target) # tar tensor([0, 0, 0,  ..., 0, 0, 0]

        # raise ValueError('self.infonce:', self.infonce, 'logits;', logits.size(), 'target', target.size())
        # ValueError: ('self.infonce:', True, 'logits;', torch.Size([1665, 101]), 'target', torch.Size([1665]))
        if self.infonce:
        ...
        if torch.randperm(1000)[0]==0:
            print('loss:', loss)
            print('loss_ent_max:', loss_ent_max)

        # if self.training:
        #     loss = loss + loss_ent_max
