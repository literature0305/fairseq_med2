...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, generator_for_hyp=None, model_for_hyp=None
    ):
        """
        ...
        """
        # inference beam-search hyps for segment-level training
        if generator_for_hyp is not None and model_for_hyp is not None:
            if sample["target"].size(1) > 3:
                len_max_prefix = 0.1
                max_len_t = max(int(sample["target"].size(1)*len_max_prefix), 3)
                rand_len_t = torch.randperm(max_len_t)[0]
                if rand_len_t == 0:
                    prefix_tokens = None
                else:
                    prefix_tokens = sample["target"][:,:rand_len_t]
            else:
                prefix_tokens = None
            constraints = None
            model_for_hyp.eval()
            hypos = self.inference_step(
                generator_for_hyp,
                model_for_hyp,
                sample,
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if 'ScheduledSamplingCriterion' in criterion.__class__.__name__:
                    loss, sample_size, logging_output = criterion(model, sample, update_num=update_num)
                elif hypos is not None:
                    loss, sample_size, logging_output = criterion(model, sample, optimizer, update_num=update_num, hypos=hypos)
                else:
